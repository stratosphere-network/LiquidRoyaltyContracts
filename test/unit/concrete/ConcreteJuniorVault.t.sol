...
contract ConcreteJuniorVaultTest is Test {
    ...
    function testVaultAccountingUpdatesAutomatically() public {
        uint256 assets = 100e18;
        ...
        assertGt(shares1, 0);
        console2.log("user1 shares balance before deposit", user1Balance / 1e18);
        console2.log("user1 shares balance after deposit", shares1 / 1e18);
        console2.log("shares received for 1000usdc deposit", shares1 / 1e18);

        // Before after each deposit/withdraw we would have to call the updateVaultValue function
        // to update the vault value based on the LP price which would be a waste of gas and can fail
        // but now it is automatically updated by the _deposit and _withdraw functions

        vm.startPrank(user2);
        uint256 initialBalance = vault.balanceOf(user2);
        lpToken.approve(address(vault), assets);
        ...
        console2.log("user2 balance before deposit", initialBalance / 1e18);
        console2.log("user2 shares balance after", shares2 / 1e18);
        console2.log("shares received for 1000usdc deposit", shares2 / 1e18);

        // Test user1 withdraw
        vm.prank(user1);
        uint256 assets2 = vault.redeem(shares1, user1, user1);
        console2.log("WITHDRAWING ===");
        console2.log("Shares balance after withdraw", vault.balanceOf(user1) / 1e18);
        console2.log("Assets received for withdraw", assets2 / 1e18);

        assertEq(assets, assets2);
        assertGt(assets2, 0);

        // Test user2 withdraw
        vm.prank(user2);
        uint256 assets3 = vault.redeem(shares2, user2, user2);
        console2.log("WITHDRAWING ===");
        console2.log("Shares balance after withdraw", vault.balanceOf(user2) / 1e18);
        console2.log("Assets received for withdraw", assets3 / 1e18);

        assertEq(assets, assets3);
        assertGt(assets3, 0);
    }
