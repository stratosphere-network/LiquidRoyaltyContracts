...
import {
  ...
  ReserveRebaseExecuted as ReserveRebaseExecutedEvent,
  DepositCapUpdated as DepositCapUpdatedEvent,
...
} from "../generated/ReserveVault/ReserveVault"
import {
  User,
  Deposit,
  Withdrawal,
...
function getOrCreateProtocolStats(): ProtocolStats {
  ...
  if (stats == null) {
    ...
    stats.currentDepositCap = BigInt.fromI32(0)
    stats.totalUsers = 0
...
export function handleReserveRebase(event: ReserveRebaseExecutedEvent): void {
  let rebase = new ReserveRebase(
    event.transaction.hash.toHexString() + "-" + event.logIndex.toString()
  )
...
export function handleDepositCapUpdated(event: DepositCapUpdatedEvent): void {
  let capUpdate = new DepositCapUpdated(
    event.transaction.hash.toHexString() + "-" + event.logIndex.toString()
  )
  
  capUpdate.oldCap = event.params.oldCap
  capUpdate.newCap = event.params.newCap
  capUpdate.timestamp = event.block.timestamp
  capUpdate.blockNumber = event.block.number
  capUpdate.transactionHash = event.transaction.hash
  capUpdate.save()
  
  let stats = getOrCreateProtocolStats()
  stats.currentDepositCap = event.params.newCap
  stats.lastUpdateTimestamp = event.block.timestamp
  stats.save()
}
