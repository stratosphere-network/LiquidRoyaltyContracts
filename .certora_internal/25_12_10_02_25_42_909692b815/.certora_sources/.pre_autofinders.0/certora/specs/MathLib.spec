...
rule supplyGrowsWithIndex(uint256 totalShares, uint256 oldIndex, uint256 newIndex) {
    ...
    uint256 supplyRatio = (newSupply * mathLib.PRECISION()) / oldSupply;
    uint256 indexRatio = (newIndex * mathLib.PRECISION()) / oldIndex;
...
rule backingRatioRevertsOnZeroSupply(uint256 vaultValue) {
    ...
    ratio@withrevert = mathLib.calculateBackingRatio(vaultValue, 0);
...
rule sharesCalculationRevertsOnZeroIndex(uint256 balance) {
    ...
    shares@withrevert = mathLib.calculateSharesFromBalance(balance, 0);
