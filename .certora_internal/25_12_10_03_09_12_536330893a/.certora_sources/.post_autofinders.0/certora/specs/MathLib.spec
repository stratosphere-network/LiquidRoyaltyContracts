...
rule balanceSharesRoundtrip(uint256 depositAmount, uint256 rebaseIndex) {
    require rebaseIndex > 0;
    require rebaseIndex <= 10 * mathLib.PRECISION(); // Reasonable rebase index (up to 10x initial)
    require depositAmount > 0;
    require depositAmount < max_uint256 / mathLib.PRECISION(); // Prevent overflow
    
    // Require minimum meaningful amount: at least 1e18 (1 token with 18 decimals)
    // This excludes dust amounts that suffer from severe rounding
    require depositAmount >= mathLib.PRECISION();
    ...
    // Property: reconstructed balance should equal original (allowing for rounding down by 1)
...
rule supplyGrowsWithIndex(uint256 totalShares, uint256 oldIndex, uint256 newIndex) {
    require oldIndex > 0;
    require oldIndex <= 10 * mathLib.PRECISION(); // Reasonable old index bound
    require newIndex > oldIndex;
    require newIndex <= 10 * mathLib.PRECISION(); // Reasonable new index bound
    require totalShares > 0;
    require totalShares < max_uint256 / newIndex;
    
    // Ensure meaningful growth above rounding: difference must exceed PRECISION
    require to_mathint(totalShares) * (newIndex - oldIndex) > mathLib.PRECISION();
...
rule backingRatioIncreasesWithVault(uint256 supply, uint256 value1, uint256 value2) {
    ...
    // Avoid complete rounding to 0: values must be meaningful relative to supply
    require value1 * mathLib.PRECISION() >= supply;
    
    uint256 ratio1 = mathLib.calculateBackingRatio(value1, supply);
    uint256 ratio2 = mathLib.calculateBackingRatio(value2, supply);
    
    // Property: higher vault value → higher backing ratio (unless both round to same value)
...
rule backingRatioDecreasesWithSupply(uint256 vaultValue, uint256 supply1, uint256 supply2) {
    ...
    // Avoid complete rounding to 0: vault value must be meaningful relative to supply
    require vaultValue * mathLib.PRECISION() >= supply2;
    ...
    // Property: higher supply → lower backing ratio (unless both round to same value)
    assert ratio2 <= ratio1,
